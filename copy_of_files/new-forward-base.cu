...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    errCheck(cudaMalloc((void **) device_mask_ptr, maskSize * sizeof(float)));
    ...
    //cudaMemcpy(device_output_ptr, host_output, outputSize* sizeof(float), cudaMemcpyHostToDevice);
    errCheck(cudaMemcpy(*device_mask_ptr, host_mask, maskSize* sizeof(float), cudaMemcpyHostToDevice));

    // Useful snippet for error checking
    cudaError_t error = cudaGetLastError();
    if(error != cudaSuccess)
    {
         std::cout<<"CUDA here error: "<<cudaGetErrorString(error)<<std::endl;
         exit(-1);
     }
